...
Json::Value CCmdInitializationReceiver::Action() {
	...
	system ("echo 1 > /home/vli/debugfs/mmc0/mmc0\:0001/cardInitialization");
	/*
	ifstream infile;
	char test[16];
	int fileSize, i;
	...
	infile.open("/home/vli/Pictures/456", ios::binary | ios::in);
	if (!infile) {
		return -1;
	}
    ...
    for (i = 0; i < q; i++) {
    	if((pp=popen("sudo find /home/vli/debugfs/mmc0 -name cardInitialization","r")) == NULL){
